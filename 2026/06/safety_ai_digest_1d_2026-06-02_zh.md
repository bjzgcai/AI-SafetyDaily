...
然而，更具紧迫性的威胁来自LLM智能体。多篇论文共同描绘了一幅值得关注的图景：**《From Prompt Injection to Persistent Control》** 首次系统化定义了针对本地智能体外壳（harness）的“多步特洛伊木马攻击”，攻击者通过污染智能体可读写的文件或工具输出植入指令，这些指令在后续会话中被智能体自主加载并执行，整个过程的任何单独步骤都可能看似无害。**《Depth-Dependent Indirect Prompt Injection in Tool-Calling ReAct Agents》** 进一步细化了这种攻击，研究了在工具调用中注入位置（深度）、负载措辞和对话轮次预算对攻击成功率的影响。**《Automatically Attacking Software Reverse Engineering AI Agents》** 和**《TRACE》** 则展示了攻击者可以自动化地生成并利用此类漏洞，前者攻击了用于二进制分析的智能体，后者构建了能够诱导LLM Agent规划并执行复杂恶意操作的任务感知自进化越狱框架。
...
对齐研究正朝着更精细化、更具理论基础和更尊重用户权利的方向发展。**《Differentially Private Preference Data Synthesis for Large Language Model Alignment》** 提出的 **DPPrefSyn** 框架是一个关键进展。它生成合成的、满足差分隐私的偏好数据，用于模型的对齐后训练。作者声称，这可以缓解真实人类偏好数据中可能包含敏感信息（用户提示和人类判断）引发的隐私担忧，为隐私保护下的偏好学习提供新路径。
...
可解释性研究则致力于理解模型内部机制。**《Measuring, Localizing, and Ablating Alignment Signatures in LLMs》** 深入探究了后训练（如RLHF）是否在模型内部引入了可识别的“AI风格”特征。通过对比人类文本、基础模型和对齐模型的生成，作者发现对齐后的生成具有更低的人类语料库亲和力，并且这种特征在模型内部具有局部化的表征。**《Certified Circuits》** 则讨论机制可解释性中“电路”发现的稳定性问题，为评估电路发现结果是否受数据集偶然性影响提供理论视角。这些工作共同推动对齐从行为表层向模型内部表征的深层理解迈进。

## 隐私保护与联邦学习：理论突破与系统设计
隐私计算领域在理论优化和应用架构上均有新收获。在差分隐私理论方面，**《Optimal conversion from Rényi Differential Privacy to $f$-Differential Privacy》** 给出了基于单阶RDP隐私区域交集的转换规则的最优性证明，适用于论文设定下的有效RDP配置和一类错误水平。这为差分隐私中两种主要隐私预算度量（RDP和f-DP）之间的转换提供了更紧的理论界限。**《Fast Mixing Mechanism for Differential Privacy》** 则将高效随机化 sketch 技术（如Hadamard变换）与差分隐私优化相结合，旨在解决大规模DP线性回归等任务中的计算效率问题。
...
再者，**可解释性与安全性的关系**错综复杂。模型内部的“对齐签名”被发现后，攻击者是否可以利用这些知识进行更精准的去对齐攻击？围绕机制电路稳定性的理论研究，能否转化为可抵御对抗性操纵的审计工具，仍需后续验证。
