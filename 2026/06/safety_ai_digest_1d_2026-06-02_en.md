...
On the interpretability front, **"Certified Circuits"** studies stability guarantees for mechanistic circuit claims, addressing the concern that circuit discovery results can be brittle or dataset-specific. The work is best read as a theoretical contribution toward more reliable interpretability audits, rather than a deployment-ready auditing framework. In a different vein, **"Measuring, Localizing, and Ablating Alignment Signatures in LLMs"** investigates the "AI-like" style produced by aligned models, finding that post-training introduces or amplifies stylistic regularities that can be localized internally. This work connects alignment training to measurable changes in model outputs and representations.
